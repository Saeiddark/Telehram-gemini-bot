...
import { getUpcomingMatches, getMatchById, getTeamRecentMatches, getMatchDetails } from '../services/worldcupApi.js';
...
- گلزنان احتمالی (اگر قابل حدس باشد)`;
  ...
  // ================== دکمه جزئیات بازی (نتایج امروز) ==================
  if (data.startsWith('match_details_')) {
    const matchId = parseInt(data.replace('match_details_', ''));
    await ctx.answerCallbackQuery('در حال دریافت جزئیات...');
    try {
      const match = await getMatchDetails(matchId);
      if (!match) return ctx.reply('بازی پیدا نشد.');

      // اطلاعات اصلی
      let detailText = `⚽ ${match.homeTeam.name} ${match.score.fullTime.home} - ${match.score.fullTime.away} ${match.awayTeam.name}\n`;
      detailText += `📅 تاریخ: ${new Date(match.utcDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}\n`;
      detailText += `🕒 ساعت: ${new Date(match.utcDate).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}\n\n`;

      // گل‌ها
      if (match.goals && match.goals.length > 0) {
        detailText += `⚽ گل‌ها:\n`;
        match.goals.forEach(goal => {
          const minute = goal.minute !== null ? `${goal.minute}'` : 'N/A';
          detailText += `- ${goal.scorer.name} (${goal.team.name}) ${minute}`;
          if (goal.type === 'PENALTY') detailText += ' (پنالتی)';
          if (goal.type === 'OWN') detailText += ' (گل به خودی)';
          detailText += '\n';
        });
        detailText += '\n';
      }
      ...
      if (match.bookings && match.bookings.length > 0) {
        ...
        match.bookings.forEach(card => {
          detailText += `- ${card.player.name} (${card.team.name}) `;
          if (card.card === 'YELLOW') detailText += '🟨 زرد';
          else if (card.card === 'RED') detailText += '🟥 قرمز';
          if (card.minute) detailText += ` (${card.minute}')`;
          detailText += '\n';
        });
        detailText += '\n';
      }
      ...
      if (match.substitutions && match.substitutions.length > 0) {
        ...
        match.substitutions.forEach(sub => {
          detailText += `- ${sub.playerOut.name} ↔️ ${sub.playerIn.name} (${sub.team.name}) ${sub.minute}'\n`;
        });
      }
      ...
      console.error(err);
      await ctx.reply('خطا در دریافت جزئیات بازی.');
    }
    return;
  }

  // ================== دکمه‌های جدید جستجو و خلاصه‌سازی ==================
  if (data === 'cmd_search') {
